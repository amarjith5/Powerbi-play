...
def _json_safe(value):
    """
    pandas represents missing values as float NaN (or NaT for dates) even in
    object-dtype columns that otherwise hold strings — e.g. every open
    ticket's ClosedDate. Python's json.dumps will write these as a literal
    `NaN` token, which is NOT valid JSON: JSON.parse() throws a SyntaxError
    on it in every browser, silently breaking the *entire* response, not
    just that one field. Converting to None here makes every result
    genuinely JSON-safe, matching Power BI's own behavior of returning null
    for missing values.
    """
...
def run_dax(query: str) -> dict:
    ...
    # --- Bare table reference: EVALUATE TableName (no filter/function) ---
    if q.strip() == table_name:
    ...
    if q.upper().startswith("ROW"):
        ...
        return {"results": [{"tables": [{"rows": [{k: _json_safe(v) for k, v in row.items()}]}]}]}
    ...
    if q.upper().startswith("CALCULATETABLE"):
        ...
        rows = _json_safe_rows(result_df.to_dict(orient="records"))
        return {"results": [{"tables": [{"rows": rows}]}]}
    ...
    if q.upper().startswith("SUMMARIZE") or q.upper().startswith("TOPN(") or "SUMMARIZE(" in q.upper():
        ...
        rows = _json_safe_rows(grouped.to_dict(orient="records"))
        return {"results": [{"tables": [{"rows": rows}]}]}
