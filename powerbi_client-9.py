...
def _build_ticket_metrics() -> dict:
    ...
    # Open vs Closed status split PER creation month (fixes "stacked chart of
    # open/closed by month" style questions — tickets_by_month above only has
    # total volume, and tickets_by_status above only has the current overall
    # split, so neither can answer "how many of March's tickets are still
    # open". Computed live off the same dataframe every other metric here
    # uses, not hardcoded, so it reflects whatever's actually in the dataset.
    # Every (month, status) combo is filled in via unstack(fill_value=0), so
    # the frontend chart always gets a consistent two-series shape per month
    # rather than a sparse dict missing a status in some months.
    month_status_counts = (
        df_dates.groupby(["_month", "Status"]).size().unstack(fill_value=0)
    )
    for status in ("Open", "Closed"):
        if status not in month_status_counts.columns:
            month_status_counts[status] = 0
    tickets_open_closed_by_month = {
        month: {"Open": int(row["Open"]), "Closed": int(row["Closed"])}
        for month, row in month_status_counts.sort_index().iterrows()
    }
    ...
    return {
        ...
        "tickets_open_closed_by_month": tickets_open_closed_by_month,
