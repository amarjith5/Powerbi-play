...
Set "needs_records": false for questions answerable from a single total, a simple one-column
breakdown, an average, or a top-N ranking — those don't need row-level data.
...
async def extract_query_filters(message: str, table: str) -> dict:
    """
    Cheap, fast LLM call that decides if a question needs a live filtered
    query and, if so, extracts which filters apply. Fails safe: any error
    or malformed response is treated as "no live query needed" so a bad
    extraction never blocks the normal aggregate-metrics answer path.
    """
    ...
    try:
        ...
        return {"needs_records": bool(parsed.get("needs_records")), "filters": clean_filters}
    except Exception as e:
        print(f"\n⚠️ extract_query_filters failed (defaulting to no live query): {e}\n")
        return {"needs_records": False, "filters": {}}
